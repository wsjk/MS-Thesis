...
for peaked = 1:1
    ...
    peaked
    LL_type = 10; %10 kip, 15 kip, 20 kip, 25 kip restraints
    type = 1; %1 = chain, 2 = nylon, use type 1 for nylon strap only
    tearstretch = 0; %stretch in tear webbing prior to activation [m] (0.33")
    nylonstretch = 0.18; %percent of effective length stretch at activation 
    vert = 30;
    horz = 30;
    lanyard = 9; %in feet
    longdisplacement = 0.2;
    latdisplacement = 0.2;
...
%     restraints = [1 2 3 4 1 2 2 3 2 3 5 6 1 2 3 3 2 4 5 7 1 2 3 4 2 4 5 7
%     1 2 3 4];
    restraints = [1 2 3 4 2 4 5 7 2 4 5 7 3 5 7 9 1 2 3 4 2 3 4 6 2 3 4 6 2 4 6 9 1 2 3 4 1 3 4 6 2 3 4 6 2 4 6 9];
    noMB1 = restraints(1,peaked);
...
% for sign = 1:4
    ...
    crashY = 16*g;
    stopper = 1;
    cargoweight = [5000 10000 15000 20000]; 
    m2 = (cargoweight(1,sign))*4.4482216/9.81; %cargo mass [kg]
    % noMB1 = 18;
...
%         v0y2 = 3.2;
        ...
        v0z2 = 0;
        crashX = 0;
        crashY = crashY - 1*g; %magnitude of peak of decel.
        crashY/g
        pulse = 0;
        if crashY > 0
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    maxstroke = 0; %max stroke of load limiter in FEET!!!!!
%     type = 2; %1 = chain, 2 = nylon
...
%     lanyard = 9; %in feet
    ...
    switch LL_type
        case 0
            RipForce = 44482; %10 kip nylon strap
            K1 = 44482/(nylonstretch*lanyard*.3048); %10 kip stiffness
        case 10
    ...
    if type == 2
        K1 = RipForce / (tearstretch + nylonstretch*lanyard*.3048);
    end
    ...
    % GRID = [-GRIDx 0 0 0 0 0 RipForce*noMB1 K1*noMB1 1];
%     GRID = [-GRIDx  W/2  0  -b/2  a/2  h RipForce*noMB1 K1*noMB1 1;... 
%             -GRIDx -W/2  0  -b/2 -a/2  h RipForce*noMB1 K1*noMB1 1;...
%              GRIDx  W/2  0   b/2  a/2  h RipForce*noMB1 K1*noMB1 0;...
%              GRIDx -W/2  0   b/2 -a/2  h RipForce*noMB1 K1*noMB1 0];
    ...
    for p = 1:4
        ...
        [t,x,te,xe,ie] = ode45(@GuidanceSystemSimulation, tspan, x0, options);
        nt = length(t);
        time = [time;t(2:end)];
...
%         end
    ...
    if type == 1
        maxstroke = ones(1,n)*max(max(newstroke));
    elseif type == 2
        maxstroke = ones(1,n)*abs((max(max(newstroke)) - (lanyard*nylonstretch*conv + tearstretch)))/conv; %still in feet
    end

    %maxstroke(j) is in feet
    for j = 1:n
        if GRID(j,end) == 1
    ...
    if direction == 'long'
        records(track,:) = [cargoweight(1,sign) 1 crashX/g max(max(newstroke)) noMB1 RipForce LL_type 2*totalweight 2*totallanyardweight 0 min(u2-u1) max(x2-x1) type 4*noMB1]
    elseif direction == 'late'
        records(track,:) = [cargoweight(1,sign) 2 crashY/g max(max(newstroke)) noMB1 RipForce LL_type 2*totalweight 2*totallanyardweight 0 min(v2-v1) max(y2-y1) type 4*noMB1]
    end
    track = track+1;
% end
records
