...
%%%%%%%%%%%%%% CRASH CHARACTERISTICS %%%%%%%%%%%%%%%
    ...
    type = 2; %1 = chain, 2 = nylon, use type 1 for nylon strap only
...
%     tearstretch = 0;
    ...
    m2 = (10000)*4.4482216/9.81; %cargo mass [kg]
...
%         crashY = decel(1,4)*g;%magnitude of peak of decel.
        crashY = decel(1,1)*g;%magnitude of peak of decel.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    switch LL_type
        case 5
            RipForce = 0; %10 kip nylon strap
            K1 = 2653424; %10 kip stiffness
        case 5
            RipForce = 22241; %10 kip nylon strap
            K1 = 2653424; %10 kip stiffness
        case 10
...
%              GRIDx -W/2  0   b/2 -a/2  h RipForce*noMB1 K1*noMB1 0];
    ...
    elseif type == 2 
        for k = 1:n
                if GRID(k,end) == 1
                    ...
                    s0(k,1) = sqrt((GRID(k,4)-GRID(k,1)+x02)^2 + (GRID(k,5)-GRID(k,2)+y02)^2 +...
                    (GRID(k,6)-GRID(k,3)+z02)^2);
                    lanyardweight(k,1) = nylon;
                    if GRID(k,7) == RipForce1*noMB1
                        lanyardweight(k,1) = 2*nylon; %nylon is originally for 5 kip so scale up
                    elseif GRID(k,7) == RipForce1*noMB1*1.5
    ...
    max(max(newstroke))
    if type == 1
        maxstroke = ones(1,n)*max(max(newstroke))/conv; %still in feet
    elseif type == 2
    ...
    if direction == 'long'
        records(1,:) = [m2/(4.4482216/9.81) 1 crashX/g maxstroke(1) noMB1 RipForce LL_type 2*totalweight 2*totallanyardweight 2*totalrestraintweight min(u2-u1) max(x2-x1) type 4*noMB1]
    elseif direction == 'late'
        records(1,:) = [m2/(4.4482216/9.81) 2 crashY/g maxstroke(1) noMB1 RipForce LL_type 2*totalweight 2*totallanyardweight 2*totalrestraintweight min(v2-v1) max(y2-y1) type 4*noMB1]
    end
