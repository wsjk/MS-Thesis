...
count = 1;
check = 1;
records = zeros(1000,16);
track = 1;

%%%%%%%%%%%%%% CRASH CHARACTERISTICS %%%%%%%%%%%%%%%
% for speed = 1:5
speed = 0;
% for disp = 0.1:0.1:1.5
    disp = 1;
    for peaked = 5:8
...
%     longdisplacement = 0.3;
    longdisplacement = disp;
    latdisplacement = longdisplacement;
...
%         v0y2 = 3.2;
        ...
        v0z2 = 0;
        crashX = 0;
        crashY = decel(1,accel)*g;%magnitude of peak of decel.
...
%     m2 = (cargoweight(1,sign))*4.4482216/9.81; %cargo mass [kg]
    cargoweight = 1000*sign;
    m2 = (cargoweight)*4.4482216/9.81; %cargo mass [kg]
...
%         end
    ...
    if direction == 'long'
        records(track,:) = [cargoweight 1 crashX/g longdisplacement v0x1 maxstroke(1) noMB1 RipForce LL_type 2*totalweight 2*totallanyardweight 2*totalrestraintweight min(u2-u1) max(x2-x1) type 4*noMB1]
    elseif direction == 'late'
        records(track,:) = [cargoweight 2 crashY/g latdisplacement v0y1 maxstroke(1) noMB1 RipForce LL_type 2*totalweight 2*totallanyardweight 2*totalrestraintweight min(v2-v1) max(y2-y1) type 4*noMB1]
    end
    
    save guidance.mat
    save('testing.mat', 'records')
    save('C:\Documents and Settings\wzk103\Desktop\RECORDS','records','-ASCII')
